...
import { updateGrupo, Grupo } from '@/lib/supabase';

type EditarGrupoModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  grupo: Grupo;
...
export default function EditarGrupoModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateGrupo(grupo.id, descricao);
      onSuccess();
    } catch (error: any) {
      setErro('Erro ao atualizar composição: ' + (error.message || 'Erro desconhecido'));
    } finally {
      setCarregando(false);
    }
  };
