...
import { updateItemOrcamento, ItemOrcamento } from '@/lib/supabase';

type EditarItemOrcamentoModalProps = {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  item: ItemOrcamento;
...
export default function EditarItemOrcamentoModal({
  isOpen,
  onClose,
  onSuccess,
  item,
  grupoNome
}: EditarItemOrcamentoModalProps) {
  const [descricao, setDescricao] = useState('');
  const [unidade, setUnidade] = useState('');
  const [quantidade, setQuantidade] = useState<string>('');
  const [precoUnitario, setPrecoUnitario] = useState<string>('');
  const [bdiPercentual, setBdiPercentual] = useState<string>('');
  ...
  useEffect(() => {
    if (isOpen && item) {
      ...
      // Calcular BDI percentual
      const bdiCalculado = (item.com_bdi / item.total - 1) * 100;
      setBdiPercentual(bdiCalculado.toFixed(2));
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      setErro('Erro ao atualizar item de orçamento. Tente novamente.');
    } finally {
      setSalvando(false);
    }
  }
